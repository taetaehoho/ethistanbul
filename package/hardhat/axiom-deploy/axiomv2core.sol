...
import {BLOCK_BATCH_DEPTH, BLOCK_BATCH_SIZE, HISTORICAL_BLOCK_BATCH_SIZE, HISTORICAL_NUM_ROOTS, getAuxMmrPeak, getBoundaryBlockData} from "../libraries/configuration/AxiomV2Configuration.sol";

/// @title  AxiomV2CoreHistoricalMock
/// @notice Mock version of core Axiom smart contract that verifies the validity of historical block hashes using SNARKs.
///         This version verifies proofs of recent block hash updates but not the historical import.
/// @dev    For use in a UUPS upgradeable contract.
contract AxiomV2CoreHistoricalMock is
    IAxiomV2Core,
    AxiomAccess,
    UUPSUpgradeable
{
    ...
    function updateRecent(
        ...
        if (root == bytes32(0)) {
            ...
            for (uint256 round; round < BLOCK_BATCH_DEPTH; ) {
                bytes32 peak = getAuxMmrPeak(
                    proofData,
                    BLOCK_BATCH_DEPTH - 1 - round
                );
                if (peak != 0) {
        ...
        if (pmmr.size >= startBlockNumber && pmmr.size <= endBlockNumber) {
            ...
            uint32 peaksChanged = pmmr.updatePaddedLeaf(
                BLOCK_BATCH_SIZE,
                root,
                numFinal
            );
            ...
            emit PaddedMerkleMountainRangeUpdated(
                blockhashPmmrKeccak,
                pmmr.size
            );
        }

        historicalRoots[startBlockNumber] = keccak256(
            abi.encodePacked(prevHash, root, numFinal)
        );
    ...
    function updateOld(
        ...
        if (
            historicalRoots[endBlockNumber + 1] !=
            keccak256(abi.encodePacked(endHash, nextRoot, nextNumFinal))
        ) {
            revert BlockHashIncorrect();
        }
        ...
        historicalRoots[startBlockNumber] = keccak256(
            abi.encodePacked(prevHash, root, BLOCK_BATCH_SIZE)
        );
    ...
    function updateHistorical(
        ...
        bytes32[BLOCK_BATCH_DEPTH + 1][HISTORICAL_NUM_ROOTS - 1]
            calldata endHashProofs,
        bytes calldata proofData
    ) external onlyProver onlyNotFrozen {
        (
            bytes32 _prevHash,
        ...
        if (
            endBlockNumber - startBlockNumber != HISTORICAL_BLOCK_BATCH_SIZE - 1
        ) {
            revert IncorrectNumberOfBlocks();
        }
        ...
        if (
            historicalRoots[endBlockNumber + 1] !=
            keccak256(abi.encodePacked(_endHash, nextRoot, nextNumFinal))
        ) {
            revert BlockHashIncorrect();
        }
        ...
        for (uint256 i; i < HISTORICAL_NUM_ROOTS; ) {
            if (i != HISTORICAL_NUM_ROOTS - 1) {
                ...
                for (uint256 j; j < BLOCK_BATCH_DEPTH; ) {
                    proofCheck = Hash.keccak(
                        endHashProofs[i][BLOCK_BATCH_DEPTH - 1 - j],
                        proofCheck
                    );
                    unchecked {
                        ++j;
            ...
            bytes32 prevHash = i == 0
                ? _prevHash
                : endHashProofs[i - 1][BLOCK_BATCH_DEPTH];
            uint32 start = uint32(startBlockNumber + i * BLOCK_BATCH_SIZE);
            historicalRoots[start] = keccak256(
                abi.encodePacked(prevHash, roots[i], BLOCK_BATCH_SIZE)
            );
            emit HistoricalRootUpdated(
                start,
                prevHash,
    ...
    function appendHistoricalPMMR(
        ...
        for (uint256 i; i < roots.length - 1; ) {
            bytes32 commitment = keccak256(
                abi.encodePacked(prevHashes[i], roots[i], BLOCK_BATCH_SIZE)
            );
        ...
        uint32 peaksChanged = pmmr.appendCompleteLeaves(
            BLOCK_BATCH_SIZE,
        ...
        uint32 leafPeaksChanged = pmmr.updatePaddedLeaf(
            BLOCK_BATCH_SIZE,
            roots[roots.length - 1],
            lastNumFinal
        );
    ...
    function upgradeHistoricalSnarkVerifier(
        address _historicalVerifierAddress
    ) external onlyRole(TIMELOCK_ROLE) {
        if (_historicalVerifierAddress == address(0)) {
            revert HistoricalVerifierAddressIsZero();
    ...
    function blockhashPmmrPeaks()
        external
        view
        returns (MerkleMountainRange.MMR memory)
    {
    ...
    function fullBlockhashPmmr()
        external
        view
        returns (PaddedMerkleMountainRange.PMMR memory)
    {
    ...
    function isBlockHashValid(
        ...
        return
            merkleRoot ==
            keccak256(
                abi.encodePacked(witness.prevHash, root, witness.numFinal)
            );
    }
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IAxiomV2Core).interfaceId ||
